...
export function ModalTrainingBar({ initialGlobalFiles, globalFiles, creditorFiles, operatorFiles, index, getFileCallBack }: IWorkoutFiles) {
    ...
    function handleOperatorFiles(Id_File_Operator: number, YoutubeExternalVideo: string | null, getOperatorFiles: Function, FileExtension: string | null | undefined) {

        return getFileCallBack(Id_File_Operator, YoutubeExternalVideo, getOperatorFiles, String(FileExtension))
    }

    function handleCreditorFiles(Id_File_Creditor: number, YoutubeExternalVideo: string | null, getCreditorFiles: Function, FileExtension: string | null | undefined) {

        return getFileCallBack(Id_File_Creditor, YoutubeExternalVideo, getCreditorFiles, String(FileExtension))
    }

    function handleGlobalFiles(Id_Global_File: number, YoutubeExternalVideo: string | null, getGlobalFiles: Function, FileExtension: string | null | undefined) {
    ...
    return (
        <>
            {index == 0 ?
                ...
                scrollbar-thin border-solid border-[1px] border-blue-500 rounded-md hover:bg-[--hover-light-rout dark:bg-transparent mb-1`}>
                        <details
                    ...
                    {creditorFiles.length > 0 || operatorFiles.length < 0 || globalFiles.length < 0 && (
                        <div
                            className={`flex flex-col gap-1 scroll-smooth overflow-y-auto
                            scrollbar-thumb-slate-400 scrollbar-track-[--bg-dark-sidebar]
                            ...
                            >
                                {operatorFiles.map((item, i) => {
                                    return (
                                        <div key={i} className={`flex flex-col gap-1 pl-8`}>
                                            <span
                                                className={`w-[16rem] p-1 flex items-center gap-3 cursor-pointer`}
                                            >
                                                {folder == "open" ? (
                                                    <FontAwesomeIcon icon={faFolderOpen} />
                                                ) : (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <p className={`w-full`}>{workoutPhaseName[index - 1]}</p>
                                    </span>
                                </summary>
                            </details>
                        </div>
